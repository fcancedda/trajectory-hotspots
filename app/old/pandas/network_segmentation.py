...
def user_to_user_distance_euclid(time, x1, x2, y1, y2):
    return np.sqrt((x2(time) - x1(time)) ** 2 + (y2(time) - y1(time)) ** 2)


def throughput_analysis(tiles, delta_s, delta_t, step, max_tp):
    # for tile, group in group_in_tile:
    # print(group.count())
    # tile = tiles['tile_key'].sort(inplace=False).unique()  # [:5]
    for tile, group in tiles.groupby('tile_key'):  # df[df['model'].isin(tile)].groupby('model')
        if group['user'].nunique() > 1:
            print("{} users in tile {} ".format(group['user'].nunique(), tile))
            user_segments_in_tile = {}
            # x_tile_key, y_tile_key, t_tile_key
            x_key, y_key, t_key = tile.split("_")
            bounding_tiles = find_bounding_tiles(float(x_key), float(y_key), float(t_key), int(delta_s))
            for surrounding_tile in bounding_tiles:
                # print(group)
                if group['tile_key'].isin(list(surrounding_tile)).any():
                    m = group['tile_key'].isin(list(surrounding_tile)).any()
                    print("found {} in bounding tile".format(m))
                    group.append(m, ignore_index=True)
                    print(group)
    ...
    #     else:
    #         user
    #         _tracker_d[current_user].append(data)
    ...
    #     continue
    # else:
    #     ___GRAB SPLINE DURING INTERACTION #
