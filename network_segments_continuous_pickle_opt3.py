...
def throughput_analysis(tiles, delta_s, delta_t, step, max_tp):
    ...
    for tile_key, tile_points in tiles.items():
        ...
        if number_of_points > 1:
            ...
            print("number of points in tile: {}".format(number_of_points))
            for pt_i in range(number_of_points):
                current_user = tile_points[pt_i][0]
                if current_user not in user_tracker_d:
                    user_tracker_d[current_user] = [(tile_points[pt_i][1], tile_points[pt_i][2],
                                                     tile_points[pt_i][3], tile_points[pt_i][4],
                                                     tile_points[pt_i][5])]
                else:
                    user_tracker_d[current_user].append((tile_points[pt_i][1], tile_points[pt_i][2],
                                                         tile_points[pt_i][3], tile_points[pt_i][4],
                                                         (tile_points[pt_i][5])))
                ...
                if surrounding_key in tiles:  # if tile exists
                    ...
                    for bounding_pt_i in range(len(points_in_b_tile)):
                        ...
                        if current_user in user_tracker_d:
                            initial_user_pid = user_tracker_d[current_user][0][3]
                            final_user_pid = user_tracker_d[current_user][-1][3]
            ...
            for combo in combinations(splines, 2):
                ...
                if max(t1) < min(t2) or max(t2) < min(t1):  # check interaction time overlaps
                    ...
                    # ----> CONTINUOUS  (computationally complex)
...
def main():
    ...
    df = pd.DataFrame.from_dict(data=tiles.items())
    # print(df.head())
    df.to_csv('generated_tiles.csv')
