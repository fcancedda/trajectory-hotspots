...
filename = 'fragments/fragments_ds1000_dt1200.csv'
...
def generate_basemap(default_location=None, default_zoom_start=12):
    # base_map = folium.Map(location=default_location, tiles='http://{s}.tiles.mapbox.com/v3/{{ API_key }}/{z}/{x}/{
    # y}.png', API_key='sk.eyJ1Ijoic2FudG9zaGdvbGkiLCJhIjoiY2p2MnVjcHpmMjkwMzN5cHBwbnhzcnQ4NCJ9
    # .EPrfebDp9_jQ2grLGnjn8w', control_scale=True, attr='XXX Mapbox Attribution', zoom_start=default_zoom_start)
    ...
    if default_location is None:
        default_location = [39.913818, 116.363625]
    map_mapbox_custo = folium.Map(
        location=default_location,
        zoom_start=default_zoom_start,
        tiles='https://api.mapbox.com/v4/mapbox.run-bike-hike/{z}/{x}/{'
              'y}.png?access_token=pk.eyJ1Ijoic2FudG9zaGdvbGkiLCJhIjoiY2p2MnRma20yMjh4cDQ0bWYxejc2ejVlcSJ9'
              '.ZuPPlyxSXQ511rvFCHl5mw',
        attr='helloo')
...
def create_visual(curr_date):
    ...
    base_map_general = generate_basemap(default_zoom_start=11)
    HeatMap(data=df_copy[['Avg_Lat', 'Avg_Lng', 'count']].groupby(
        ['Avg_Lat', 'Avg_Lng']).sum().reset_index().values.tolist(), radius=12, max_zoom=11, min_opacity=0.5).add_to(
        base_map_general)
    ...
    for hour in df_copy.hour.sort_values().unique():
        df_hour_list.append(df_copy.loc[df_copy.hour == hour, ['Avg_Lat', 'Avg_Lng', 'count']].groupby(
            ['Avg_Lat', 'Avg_Lng']).sum().reset_index().values.tolist())
    # print(df_hour_list)
    base_map = generate_basemap(default_zoom_start=11)
...
def create_general_visual(file_name):
    df_copy = df.copy()  # y-m-d
    df_copy['count'] = 1

    base_map_general = generate_basemap(default_zoom_start=11)
    HeatMap(data=df_copy[['Avg_Lat', 'Avg_Lng', 'count']].groupby(
        ['Avg_Lat', 'Avg_Lng']).sum().reset_index().values.tolist(), radius=10, max_zoom=11, min_opacity=0.3).add_to(
        base_map_general)
    base_map_general.save('heatmapsbydate/foli1_heat_general_' + str(file_name.split('/')[1]) + '.html')
